...
NSMutableArray* insertionSort(NSArray* array) {
  ...
  for (int i = 1; i < len; i++) {
    NSNumber *swapNumber = [sortedArray objectAtIndex:i];
    int swapInt = [swapNumber intValue];
    int j = i;

    while (j > 0 && [[sortedArray objectAtIndex:(j - 1)] intValue] > swapInt) {
      [sortedArray replaceObjectAtIndex:j withObject:[sortedArray objectAtIndex:(j - 1)]];
      j--;
    }

    if (i != j) {
      [sortedArray replaceObjectAtIndex:j withObject:swapNumber];
    }
  }
...
int main (int argc, const char * argv[]) {
  ...
  NSMutableArray *sortedArr = insertionSort(arr);
